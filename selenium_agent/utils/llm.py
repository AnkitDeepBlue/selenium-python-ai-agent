...
def normalize_provider(provider: str | None) -> str:
    """Validate and normalize the configured LLM provider."""
...
def get_api_key_env_var(provider: str) -> str:
    """Return the environment variable used for the provider API key."""
    normalized = normalize_provider(provider)
    return API_KEY_ENV_VARS[normalized]


def format_missing_api_key_error(provider: str) -> str:
    """Build a provider-specific missing API key message."""
    normalized = normalize_provider(provider)
...
class BaseLLMClient:
    """Minimal interface shared by provider implementations."""
...
class AnthropicLLMClient(BaseLLMClient):
    """Anthropic-backed text generation client."""
...
class OpenAILLMClient(BaseLLMClient):
    """OpenAI-backed text generation client using the Responses API."""
...
def create_llm_client(provider: str, api_key: str, model: str | None = None) -> BaseLLMClient:
    """Create a provider-specific client wrapper."""
    normalized = normalize_provider(provider)
